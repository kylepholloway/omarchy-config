...
BarWidget {
  ...
  function isSlack(item) {
    // Slack exposes its unread-state text as the tooltip title and leaves its
    // app title blank, so normalize both its icon and user-facing name here.
    return TrayModel.itemNamed(item, "slack")
  }

  function trayIcon(item) {
    if (isSlack(item)) {
      var slackIcon = Quickshell.iconPath("slack", true)
      if (slackIcon) return slackIcon
    }
    return item.icon
  }

  function trayDisplayName(item) {
    if (isSlack(item)) return "Slack"
    ...
    Column {
      ...
      Repeater {
        ...
        delegate: Item {
          ...
          readonly property string displayName: root.trayDisplayName(modelData)
          ...
          TrayIcon {
            ...
            width: 16
            height: 16
            icon: root.trayIcon(rowRoot.modelData)
          }
  ...
  component TrayItem: Item {
    ...
    TrayIcon {
      ...
      icon: root.trayIcon(trayItemRoot.modelData)
    }
